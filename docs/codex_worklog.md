...
- 为了让接手者能实际登录服务器运行剩余代码，新增 `docs/handover/SERVER_QUICKSTART.md`：
  - 新 8 卡 NPU 的登录、docker、conda、环境变量、训练完成检查。
  - 250 epoch checkpoint/config/text cache 路径检查。
  - holdout_v2 balanced/medium 的 smoke 与 full 200 eval 命令模板。
  - 3090 的登录、Python 环境、readable50 输出定位。
  - OCR CER/NED readability 二次计算命令模板。
