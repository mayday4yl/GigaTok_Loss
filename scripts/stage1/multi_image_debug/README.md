...
r030 是正常强 mask，r070 是 oracle-like 强遮挡机制 probe。
...
python3 - "$R030_EVALMASK_CONFIG" "$R030_NOMASK_CONFIG" "$R070_EVALMASK_CONFIG" "$R070_NOMASK_CONFIG" <<'PY'
import sys
import yaml
...
## 5. correct / empty / shuffled sensitivity
...
for MASK_TAG in nomask evalmask; do
  for TEXT_MODE in correct empty shuffled; do
...
- `correct ≈ shuffled < empty`：主要使用“有文本”，具体文本内容依赖不足。
- `correct ≈ shuffled ≈ empty`：text branch 仍没有形成内容依赖。
- `evalmask` 有 gap、`nomask` 没 gap：说明 mask 是触发 text 使用的关键。
- r070 有 gap、r030 没 gap：说明普通 0.3 mask 强度不够，后续应改 mask/schedule。
