...
OUT=${EVAL_DIR}/ocr_readability_${STAMP}
SMOKE=${OUT}/smoke
FULL=${OUT}/full
...
echo "${OUT}" > "${EVAL_DIR}/ocr_readability_latest_path.txt"
...
PY=/data/conda/envs/train/bin/python
COMMON=(
...
ASCEND_RT_VISIBLE_DEVICES=0 "${PY}" "${COMMON[@]}" \
  --output-dir "${SMOKE}" \
  --max-images 10 \
  --sample-seed 0 \
  --shard-index 0 \
...
for shard in $(seq 0 $((N_SHARDS - 1))); do
  ...
  (
    ...
    ASCEND_RT_VISIBLE_DEVICES="${shard}" "${PY}" "${COMMON[@]}" \
      --output-dir "${FULL}" \
      --shard-index "${shard}" \
      --num-shards "${N_SHARDS}" \
      --device-id 0 \
